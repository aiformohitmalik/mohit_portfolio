...
function makeBolt(cw, midY) {
  ...
  while (dir > 0 ? cx < endX - 10 : cx > endX + 10) {
    ...
    cy  = Math.max(midY - 40, Math.min(midY + 50, cy)); // stay in cloud body
    pts.push({ x: cx, y: cy });
  }
...
function makeBranches(bolt, cw, midY, count) {
  ...
  for (let b = 0; b < count; b++) {
    // Pick a random point along the main bolt (avoid very start/end)
    ...
    // 50 % chance of a sub-branch from this branch
...
function triggerStrike(clouds, cw) {
  ...
  // Mark every cloud the main bolt OR any branch passes through
...
function drawCloud(ctx, cloud, cw) {
  ...
  // Per-cloud lightning glow — lights up when bolt passes through
  if (cloud.glow > 0.01) {
    ctx.save();
    ctx.globalCompositeOperation = 'screen';
...
export const MatrixRain = () => {
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      // Clouds — each lights up individually via cloud.glow
      for (const cloud of clouds) drawCloud(ctx, cloud, canvas.width);

      // 3. Bolt lines ON TOP of clouds (screen blend — appears inside cloud mass)
      if (strike) {
